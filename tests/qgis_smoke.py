...
class PluginTests(unittest.TestCase):
    ...
    def test_gui_and_scoring_hooks(self):
        ...
        self.assertEqual(win.credit_label.text(), "河北地质大学 ★ 高科科\nQQ:996517087")
        ...
        self.assertEqual(win.width(), 464)
        self.assertLessEqual(win.height(), 520)
        restart_pos = win.btn_restart.mapTo(win, QPoint(0, 0))
        pause_pos = win.btn_pause.mapTo(win, QPoint(0, 0))
        preview_pos = win.preview_area.mapTo(win, QPoint(0, 0))
        panel_pos = panel.mapTo(win, QPoint(0, 0))
        self.assertEqual(restart_pos.y(), pause_pos.y())
        self.assertEqual(win.btn_restart.width(), 88)
        self.assertEqual(win.btn_pause.width(), 88)
        self.assertGreater(pause_pos.x(), restart_pos.x() + win.btn_restart.width())
        preview_gap = restart_pos.y() - preview_pos.y() - win.preview_area.height()
        self.assertGreaterEqual(preview_gap, 2)
        self.assertLessEqual(preview_gap, 10)
        self.assertGreaterEqual(panel_pos.y(), github_pos.y() + win.github_button.height())
        self.assertGreaterEqual(panel_pos.x(), 244)
        self.assertLessEqual(panel_pos.x() + panel.width(), win.width() - 20)
        table_bottom = panel.table.mapTo(win, QPoint(0, panel.table.height())).y()
        self.assertLessEqual(table_bottom, win.height() - 20)
        ...
        self.assertFalse(qt_errors)
        output = Path(__file__).resolve().parents[1] / "qgis4-two-column-preview.png"
